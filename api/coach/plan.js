...
${courseIntel && courseIntel.terrain && courseIntel.terrain !== 'flat' ? `- Run/course profile: ${courseIntel.terrain} — prescribe hill repeats and downhill running practice in the build phase; long runs should rehearse the elevation profile.` : courseIntel && courseIntel.terrain ? '- Run/course profile: flat — neutral pacing, no hill-specific work needed' : ''}
${courseIntel && courseIntel.bikeProfile && courseIntel.bikeProfile !== 'none' && courseIntel.bikeProfile !== 'flat' ? `- BIKE leg profile: ${courseIntel.bikeProfile} — include weekly climbing intervals (3-6×3-5 min @ threshold up a real climb or trainer 8-10% grade), long bike rehearses sustained climbing, brick simulates climb-to-run transition.` : courseIntel && courseIntel.bikeProfile === 'flat' ? '- BIKE leg profile: flat/fast — emphasize sustained power on flat terrain (over-unders, sweet-spot intervals); aero position practice in long bike.' : ''}
${courseIntel && courseIntel.surface && courseIntel.surface !== 'road' ? `- Surface: ${courseIntel.surface} — include surface-specific work: trail = 20-30% of weekly run on uneven terrain + ankle/glute strength; gravel = bike-handling drills + lower tire pressure practice.` : ''}
${courseIntel && courseIntel.swim && courseIntel.swim !== 'none' && courseIntel.swim !== 'pool' ? `- SWIM venue: ${courseIntel.swim} (open water) — include sighting drills (every 4th stroke), at least 2 open-water swim sessions in the final 6 weeks, wetsuit practice if cold, ${courseIntel.swim === 'ocean' ? 'chop/wave rough-water work' : courseIntel.swim === 'river' ? 'current/sighting practice in moving water' : 'group-swim drafting practice'}.` : courseIntel && courseIntel.swim === 'pool' ? '- SWIM venue: pool — standard CSS-anchored intervals, no open-water-specific work needed.' : ''}
${courseIntel && courseIntel.climate && (courseIntel.climate === 'hot' || courseIntel.climate === 'hot-humid') ? `- Race climate: ${courseIntel.climate} — schedule a 10-14 day heat-acclimatization block in the final 4 weeks (sauna 20-30 min post-easy-session, or overdressed Z2 runs). Most heat-prep gains come in 14 days; benefits last 2-3 weeks.` : courseIntel && courseIntel.climate === 'cool' ? '- Race climate: cool — neutral, no heat prep needed.' : ''}
${courseIntel && courseIntel.altitude && courseIntel.altitude !== 'sea' ? `- Altitude: ${courseIntel.altitude === 'high' ? 'high (6,500ft+) — if user can\'t pre-acclimatize, plan a "race below LT" strategy. Suggest arriving 1-2 days OR 14+ days out, never 4-7 days (worst window).' : 'moderate (2,000-6,500ft) — minor effect; pace 2-4% slower at altitude. Heat acclimatization helps acclimatize to altitude too.'}` : ''}
${courseIntel && courseIntel.notes ? `- Course notes: ${courseIntel.notes} — work this specifically into the plan rationale and a session prescription where relevant.` : ''}
